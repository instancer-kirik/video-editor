class MobileVideoRecorder {
    ...
    setupEventListeners() {
        ...
        // Zoom controls
        document.getElementById('zoom-1x').addEventListener('click', () => this.setZoom(1));
        document.getElementById('zoom-2x').addEventListener('click', () => this.setZoom(2));
        document.getElementById('zoom-5x').addEventListener('click', () => this.setZoom(5));
        ...
        document.getElementById('framerate-select').addEventListener('change', (e) => {
            ...
            navigator.getBattery().then(battery => {
                battery.addEventListener('levelchange', () => {
                    if (battery.level < 0.1 && this.isRecording) {
                        this.showToast('Low battery - consider stopping recording', 'error');
                    }
                });
            });
        }
    ...
    async switchCamera() {
        try {
            ...
            await this.initializeCamera();
            this.showToast(`Switched to ${this.currentCamera === 'user' ? 'front' : 'back'} camera`, 'success');
        } catch (error) {
    ...
    updateZoomButtons() {
        ...
        document.getElementById(`zoom-${this.currentZoom}x`).classList.add('active');
    }
    ...
    processVideoWithWasm() {
        ...
        try {
            ...
            const clipId = exports.add_clip(0.0, this.calculateRecordingDuration() / 1000, namePtr, namePtr.length, 1);
        ...
        if (this.currentStream && !this.isRecording) {
            setTimeout(async () => {
                try {
                    ...
                    console.error('Failed to reinitialize camera after orientation change:', error);
                }
            }, 200);
        }
    }
